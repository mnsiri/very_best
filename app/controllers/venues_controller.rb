class VenuesController < ApplicationController
  def index
    ...
    @venues = @q.result(:distinct => true).includes(:bookmarks, :specialties, :fans).page(params[:page]).per(10)
    @location_hash = Gmaps4rails.build_markers(@venues.where.not(:address_latitude => nil)) do |venue, marker|
      ...
      marker.infowindow "<h5><a href='/venues/#{venue.id}'>#{venue.name_venue}</a></h5><small>#{venue.address_formatted_address}</small>"
    end
  ...
  def create
    ...
    @venue.name_venue = params[:name_venue]
